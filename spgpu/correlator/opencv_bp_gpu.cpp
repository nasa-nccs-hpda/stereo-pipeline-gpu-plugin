...
int runCorrelator(int argc, char** argv)
{
    ...
    while (argi < argc - 3) {
        ...
        else if (key == "-levels") levels = stoi(argv[++argi]);
    ...
    cout << "Running GPU StereoBeliefPropagation with parameters:\n"
         << "  num_disp=" << num_disp
         << ", iters=" << iters
         << ", levels=" << levels << endl;
    ...
    Ptr<cuda::StereoBeliefPropagation> bp = cuda::createStereoBeliefPropagation(num_disp, iters, levels);
